        ...
        fluidRow(
          ...
          box(
            ...
            conditionalPanel(
              ...
              numericInput("standard_conc", "Concentration:", value = 0, min = 0, step = 0.1),
              selectInput("standard_units", "Standard Units:", 
                         choices = c("ng/μL" = "ng_uL", "ng/mL" = "ng_mL"),
                         selected = "ng_mL")
            ),
            br(),
      ...
      tabItem(tabName = "results",
        ...
        fluidRow(
          box(
            ...
            p("Final concentrations are in ng/μL, corrected for dilution factor."),
            p("Note: If standards were in ng/mL, they were automatically converted for calculations."),
...
server <- function(input, output, session) {
  ...
  output$plate_heatmap <- renderPlotly({
    ...
    plot_data <- values$parsed_data %>%
      ...
      mutate(
        Row_num = 9 - match(Row, LETTERS),  # Reverse so A is at top
        Column_num = Column
      )
    ...
    p <- ggplot(plot_data, aes(x = Column_num, y = Row_num, fill = Fluorescence, 
      ...
      scale_y_continuous(breaks = 1:8, labels = LETTERS[8:1]) +
  ...
  observeEvent(input$apply_quick, {
    ...
    if (input$quick_type == "Standard") {
      ...
      values$well_mappings$Units[selected_rows] <- ifelse(input$standard_units == "ng_mL", "ng/mL", "ng/μL")
    } else {
      values$well_mappings$Concentration_ng_uL[selected_rows] <- NA
      values$well_mappings$Units[selected_rows] <- "ng/μL"
    }
  ...
  observeEvent(input$generate_curve, {
    ...
    standards <- merged_data %>%
      ...
      mutate(
        ...
        Concentration_corrected = ifelse(Units == "ng/mL", 
                                       Concentration_ng_uL / 1000, 
                                       Concentration_ng_uL)
      ) %>%
      group_by(Concentration_corrected) %>%
      summarise(
        Mean_Fluorescence = mean(Fluorescence, na.rm = TRUE),
        SD_Fluorescence = sd(Fluorescence, na.rm = TRUE),
        N = n(),
        Original_Conc = first(Concentration_ng_uL),
        Original_Units = first(Units),
        .groups = "drop"
      ) %>%
      mutate(SE_Fluorescence = SD_Fluorescence / sqrt(N)) %>%
      rename(Concentration_ng_uL = Concentration_corrected)
  ...
  output$standards_table <- DT::renderDataTable({
    ...
    # Create display table with original concentrations and units
    display_table <- values$standards_data %>%
      mutate(
        `Original Concentration` = paste(Original_Conc, Original_Units),
        `Concentration (ng/μL)` = round(Concentration_ng_uL, 4)
      ) %>%
      select(`Original Concentration`, `Concentration (ng/μL)`, Mean_Fluorescence, 
             SD_Fluorescence, SE_Fluorescence, N)
    
    DT::datatable(display_table, options = list(pageLength = 10)) %>%
